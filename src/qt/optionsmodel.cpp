...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeRDCTAmount"))
        settings.setValue("nAnonymizeRDCTAmount", 1000);

    nAnonymizeRDCTAmount = settings.value("nAnonymizeRDCTAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeRDCTAmount"))
        SoftSetArg("-anonymizerdctamount", settings.value("nAnonymizeRDCTAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in rdct.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRDCTAmount:
            return QVariant(nAnonymizeRDCTAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeRDCTAmount:
            nAnonymizeRDCTAmount = value.toInt();
            settings.setValue("nAnonymizeRDCTAmount", nAnonymizeRDCTAmount);
            emit anonymizeRDCTAmountChanged(nAnonymizeRDCTAmount);
            break;
        case CoinControlFeatures:
